...
def main(lr, ags, grad_clip_max_norm, lora_rank, lora_alpha, step_size):
    ...
    experiments_dir = base_path / 'experements_grad_clip'
    experiments_dir.mkdir(parents=True, exist_ok=True)

    experiment_prefix = f'experement_{lr}_{ags}_{lora_rank}_{lora_alpha}_{step_size}_'
    existing_experiments = list(experiments_dir.glob(f"{experiment_prefix}*"))

    if existing_experiments:
        existing_experiments.sort()
        OUTPUTS_PATH = existing_experiments[0]
        print(f"Resuming experiment in existing directory: {OUTPUTS_PATH}")
    else:
        OUTPUTS_PATH = experiments_dir / f'{experiment_prefix}{JOB_SUFFIX}'
    ...
    def fold_chain(sequence, pdb_id, *, model, out_dir=OUT_DIR):
        ...
        try:
            df = model.ttt(sequence, return_logs=True, correct_pdb_path=CORRECT_PREDICTED_PDB / f"{pdb_id}.pdb")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--step_size', type=int, required=True, help='Step size for ProteinTTT.')

    args = parser.parse_args()

    main(args.lr, args.ags, args.grad_clip_max_norm, args.lora_rank, args.lora_alpha, args.step_size)
