...
def check_sequence(orig_seq, pred_seq):
    """Check if residue sequences match exactly."""
    if len(orig_seq) != len(pred_seq):
        raise ValueError(f"Length mismatch: original {len(orig_seq)}, predicted {len(pred_seq)}")
...
def fix_pdb(original_pdb, predicted_pdb, chain_id, output_pdb):
    ...
    # Compare sequences
    check_sequence(orig_seq, pred_seq)
