"""
MSA sampling strategies for ProteinTTT.

Adapted from PoET (OpenProteinAI):
https://github.com/OpenProteinAI/PoET/blob/main/poet/msa/sampling.py

Reference:
    Truong Jr, T. F., & Bepler, T. (2024).
...
def _hash_of_string_list(strings: list[str]) -> str:
    """Hash a list of strings into a single SHA-1 hex digest."""
    h = hashlib.sha1()
    for s in strings:
        h.update(s.encode())
    return h.hexdigest()
...
def compute_homology_weights(
    ...
    if result_cache_dir is not None:
        ...
        result_hash = _hash_of_string_list(additional_hash_components + [result_hash])
